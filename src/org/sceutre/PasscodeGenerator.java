...
/**
 * An implementation of the HOTP generator specified by RFC 4226. Generates short passcodes that may
 * be used in challenge-response protocols or as timeout passcodes that are only valid for a short
 * period.
 * 
 ...
 */
/*
 * Extracted from google authenticator project, which is Apache 2 licensed
 */
public class PasscodeGenerator {
   ...
   // private static final int INTERVAL           = 30;  // 30s is what google authenticator uses/expects
   private static final int TETRAPOD_INTERVAL     = 450; // 7.5 mins
   ...
   public static PasscodeGenerator on(String secret) throws Exception {
      // Do the magic with the secret key
      return PasscodeGenerator.on(Base32String.decode(secret));
   }
   
   public static PasscodeGenerator on(final byte[] keybytes) throws Exception {
   ...
   public PasscodeGenerator(Mac mac) {
      this(mac, PASS_CODE_LENGTH, TETRAPOD_INTERVAL);
   }
   ...
   /**
    * Verify a timeout code. The timeout code will be valid for a time determined by the interval
    * period and the number of adjacent intervals checked.
    * 
